<template>
  <div>
    <v-row class="justify-space-between align-center">
      <v-col cols="12" md="6">
        <h1 class="primary-text">Renew Comprehensive (EZ Drive) Policy</h1>
      </v-col>
    </v-row>
...
export default defineComponent({
  ...
  setup() {
    ...
    const runSearch = async () => {
      ...
      try {
        const response = await policyStore.searchEzdrivePolicyForRenewal(reg);
    ...
    const submitRenewal = async () => {
      ...
      try {
        ...
        const response = await policyStore.renewEzdrivePolicy(id, payload);
