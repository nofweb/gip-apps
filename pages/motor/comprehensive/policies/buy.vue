...
import { usePolicyStore } from "~/store/motor/comprehensive/policy";
...
export default defineComponent({
  ...
  setup() {
    ...
    const calculate =  async () => {
        ...
        try{
            ...
            policyStore.policyData.premium = response.data
        }catch(e){
            console.log(e)
        }
    }
  ...
  const numberValue = typeof value === "number" ? value : parseFloat(value.toString().replace(/,/g, ""));
  return isNaN(numberValue) ? "" : numberValue.toLocaleString("en-NG");
};
...
const formattedCarValue = computed({
  get: () => formatCurrency(policyStore.policyData.car_value),
  set: (val) => {
    policyStore.policyData.car_value = parseCurrency(val);
    calculate(); // recalculate premium when car value changes
  }
});

const formattedPremium = computed({
  get: () => formatCurrency(policyStore.policyData.premium),
  set: () => {} // Premium is not user-editable
});
    ...
    const verifyId = async () => {
      console.log("Attempt to verify ID")
        const data = {
        ...
        try{
            ...
            if(response.status == 'success'){
                console.log("Identification verified");
            }
            else{
                errors.value = true
                const error = response?.value?.data?.message;
            console.log("There was an errror verifying ID", error);
          notify({
            title: "Error",
            text: error,
            type: "error",
    ...
    const saveVehicleDetails = async () => {
        ...
        try{
            const response = await policyStore.saveVehicleDetails(data);
            console.log("The response is", response?.value?.data)
            if(response.status == 'success'){
    ...
    const buyPolicy = async () => {
      if(policyStore.policyData.identification_selected !== ""){
        ...
        if(policyStore.policyData.variant != 'comprehensive'){
  ...
  if (errors.value) {
    ...
    return;
      ...
      console.log("Buy policy butto was clicked")
      try{
