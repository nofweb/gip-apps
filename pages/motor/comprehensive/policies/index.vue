...
export default defineComponent({
  ...
  setup() {
    ...
    const variant = ref([
      ...
      { name: "EZ PLUS", value: "EZ PLUS" },
      { name: "COMPREHENSI", value: "COMPREHENSI" },
    ]);

    // (kept, but fixed typo "nam" -> "name" and "Quaterly" -> "Quarterly")
      ...
      try {
        ...
        if (response?.status === "success") {
          ...
          totalPages.value = Math.ceil(
            (response.data.total || 0) / (response.data.per_page || pageSize.value)
          );
        } else {
          policy.value = [];
        }
    ...
    const makeFileName = () => {
      ...
      return `my_comprehensive_policies_${v}_${s}_${from}_to_${to}.csv`.replace(/\s+/g, "_");
    };
